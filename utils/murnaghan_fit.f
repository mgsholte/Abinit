...
C     dimension ipvt(4),qtf(4),wa1(4),wa2(4),wa3(4),wa4(mdata)
      external eos2,murna,murna1
      data third/0.3333333333333333d0/
C     conversion factor for bulk modulus, e.g.
C     1 Ry/(a.u.**3) = 13.61 * 1.60219 * 100  / (0.529177**3) GPa
C     1 Ry/(a.u.**3) = 13.61 * 1.60219 * 1000 / (0.529177**3) Kbar
C     1 eV/(AA**3)   = 1.60219 Mbar
C     etc.
      data bfac/147153.05/  !  volume in a.u.**3, energy in Ry, B in Kbar
c
      print *,' Give name of an input file containing data columnwise:'
      print *,' [ Unit cell volume in (a.u.)^3 ] ;  [ Energy in Ry ]'
      print *,' ( lines with # in the 1st position will be ignored )'
      read *, finame
      open (55,file=finame,status='OLD',form='FORMATTED',err=11)
      goto 12
   11 continue
      print *,' Error opening file ',finame 
      stop
   12 continue
C --- read in data till end of file:
      i=1
 1    read(55,101,end=2) string
      if (string(1:1).eq.'#') then
        goto 1     !  ignore lines beginning with  #
      else
        read(string(1:length),*,err=3) vol(i),ene(i)
      endif
C
C     vol(i)=vol(i)**3/2.d0       ! if input is bcc lattice constant (in a.u.)
...
C     ene(i)=ene(i)/13.61         ! if input energy is in eV
      ...
      goto 1
    3 continue
      print *,' Error reading data in data line ',i
      stop
    2 continue
      m=i-1
      if (m.gt.mdata) then
...
C
      print *,'  Output file for results and fit: '
      read *, foname
      open (66,file=foname,form='FORMATTED',err=21)
      goto 22
   21 continue
      print *,' Error opening file ',foname 
      stop
   22 continue
C
...
C     epsfcn=0.01d0
     ...
     *           info,nfev,fjac,ldfjac,ipvt,qtf,wa1,wa2,wa3,wa4)
      write(66,201) info
  201 format('#  Fit to Murnaghan equation of state, info code=',i4)
      write(66,202)
  202 format('#'/,'#   Input V,(a.u.)^3',
     .       5x,'Input E (Ry)',4x,'Absolute fit error')
      do j=1,m
        write(66,206)vol(j),ene(j),y(j)
      enddo
C     After fitting, x(1) is min. energy in initial units,
C                    x(2) is equilibrium volume in initial units,
C                    x(4) is d(Bulk modulus)/d(Pressure) - dimensionless.
C                    x(3) is bulk modulus and needs conversion:
      bmod = x(3) * bfac
      write(66,203) x(2),x(1),bmod,x(4)
  203 format(/,'#  Equilibrium volume, a.u.^3 :',f12.6,/,
     .         '#  Minimum energy, Ry         :',f12.6,/,
     .         '#  Bulk modulus, Kbar         :',f12.6,/,
     .         '#  BP                         :',f12.6)
C
C.....create plotfile for Murnaghan fit:
      xmin=vol(1)
      xmax=vol(m) 
      dx=(xmax-xmin)/(mdata-1)
      write(66,205)
  205 format('#'/'#  Total energy fitted to Murnahgan EOS:',/
     .       '#    Volume(a.u.^3)',7x,'Energy(Ry)',5x,'Pressure (GPa)',
     .       6x,'Enthalpy(Ry)')
      do j=1,mdata
        xx=xmin+(j-1)*dx
        call murna1(x,xx,ee,press,entha)
        write(66,206) xx,ee,press,entha
      enddo
  206 format(4f18.8)
  101 format(a256)
C
